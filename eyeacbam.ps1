...
$bv = ("bam", "bam\State")
Try{$Users = foreach($ii in $bv){Get-ChildItem -Path "HKLM:\SYSTEM\CurrentControlSet\Services\$($ii)\UserSettings\" -ErrorAction SilentlyContinue | Select-Object -ExpandProperty PSChildName}}
Catch{
...
$Bam = Foreach ($Sid in $Users){$u++
        ...
        foreach($rp in $rpath){
           ...
           Write-Host -ForegroundColor Gray "$($rp)UserSettings\$SID"
           $bi = 0
            Try{
